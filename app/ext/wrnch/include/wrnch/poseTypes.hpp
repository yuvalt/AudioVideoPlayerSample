...
{
template<>
struct default_delete<wrSerializedData>
{
    void operator()(wrSerializedData* serializedEstimator) const
    {
        wrSerializedData_Destroy(serializedEstimator);
    }
};
}  // namespace std
...
{
  public:
    explicit SerializedData(wrSerializedData* serializedEstimator)
    : m_impl(serializedEstimator)
    {
        assert(m_impl.get());
    }

    char const* Data() const { return wrSerializedData_Data(m_impl.get()); }

    int NumBytes() const { return wrSerializedData_NumBytes(m_impl.get()); }

  private:
    std::unique_ptr<wrSerializedData> m_impl;
};
...
class MutableBox2d : public BasicBox2d<wrBox2d*>
{
  public:
    ...
    explicit MutableBox2d(const wrBox2d& other)
    : Base(wrBox2d_Clone(&other))
    {
...
class MutableHandBoxPair : public BasicHandBoxPair<wrHandBoxPair*>
{
  public:
    ...
    explicit MutableHandBoxPair(const wrHandBoxPair& other)
    : Base(wrHandBoxPair_Clone(&other))
    {
        utils::CheckBadAlloc(this->m_impl);
    }
...
class BasicPose2d
{
  public:
    ...
    unsigned int GetNumJoints() const { return wrPose2d_GetNumJoints(m_impl); }
...
class MutablePose2d : public BasicPose2d<wrPose2d*>
{
  public:
    ...
    explicit MutablePose2d(int numJoints)
    : Base(wrPose2d_Create(numJoints))
    {
        utils::CheckBadAlloc(Base::Get());
    }

    explicit MutablePose2d(const wrPose2d& other)
    : Base(wrPose2d_Clone(&other))
    {
        utils::CheckBadAlloc(this->m_impl);
    }
...
{
  public:
    explicit Box3dView(const wrBox3d* ptr)
    : m_impl(ptr)
    {
    ...
    bool operator==(const Box3dView& other) const
    {
        return wrBox3d_Equals(m_impl, other.m_impl) != 0;
    }
...
class BasicPose3d
{
  public:
    ...
    unsigned int GetNumJoints() const { return wrPose3d_GetNumJoints(m_impl); }
...
class MutablePose3d : public BasicPose3d<wrPose3d*>
{
  public:
    ...
    explicit MutablePose3d(int numJoints)
    : Base(wrPose3d_Create(numJoints))
    {
        utils::CheckBadAlloc(Base::Get());
    }

    explicit MutablePose3d(const wrPose3d& other)
    : Base(wrPose3d_Clone(&other))
    {
        utils::CheckBadAlloc(this->m_impl);
    }
    ...
    explicit MutablePose3d(const BasicPose3d<const wrPose3d*>& other)
...
{
  public:
    explicit ArrowView(const wrArrow* ptr)
    : m_impl(ptr)
    {
...
class MutablePoseHead : public BasicPoseHead<wrPoseHead*>
{
  public:
    ...
    explicit MutablePoseHead(const wrPoseHead& other)
    : Base(wrPoseHead_Clone(&other))
    {
        utils::CheckBadAlloc(this->m_impl);
    }
...
class BasicPoseFace
{
  public:
    BasicPoseFace();
    explicit BasicPoseFace(PtrType other)
    : m_impl(other)
    {
        utils::CheckBadAlloc(m_impl);
    }
    ...
    typename std::remove_pointer<PtrType>::type const* Get() const { return this->m_impl; }
...
class MutablePoseFace : public BasicPoseFace<wrPoseFace*>
{
  public:
    ...
    explicit MutablePoseFace(const wrPoseFace& other)
    : Base(wrPoseFace_Clone(&other))
    {
        utils::CheckBadAlloc(this->m_impl);
    }
