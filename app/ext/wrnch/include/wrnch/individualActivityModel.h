...
extern "C"
{
    ...
    ///         Estimates activities (or "gestures") and their probabilities for an individual
    ///         person. Currently, poses cannot be passed directly to `wrIndividualActivityModel` to
    ///         update its internal state. Instead, they are passed through `wrActivityModel`
    ///         instances.
    /// @see `wrActivityModel`
    /// @see activityModel.h
    ...
    /// @brief Return the class name corresponding to a class index. If no such class index exists,
    ///         ie `classIdx < 0` or `numClasses <= classIdx`, then NULL is returned.
    WRNCH_DLL_INTERFACE char const* wrIndividualActivityModel_ClassName(
        wrIndividualActivityModelHandleConst activityModel, int classIdx);
