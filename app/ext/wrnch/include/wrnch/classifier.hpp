...
class Classifier
{
  public:
    ...
    SerializedData Serialize() const { return SerializedData{ wrClassifier_Serialize(Get()) }; }

    /// @brief Attempt to deserialize a serialized classifier. Failures to deserialize result in
    ///         an exception being thrown. Note that the serializedData should be from a classifier
    ///         serialized on an identical device (including driver versions) using
    ///         `Classifier::Serialize`. If this
    ///         fails to be the case, then an exception is raised. In the sucessful case, any
    ...
    /// @note Not supported on Apple platforms.
    static Classifier Deserialize(char const* data,
                                  int numBytes,
                                  int deviceId = 0,
                                  ...
                                  const std::string& licensePath = std::string())
    {
        ...
        wrReturnCode const code = wrClassifier_DeserializeWithLicenseData(
            data,
            numBytes,
            deviceId,
    ...
    wrClassifier* Get() { return m_impl; }
